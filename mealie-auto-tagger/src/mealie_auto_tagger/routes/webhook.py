...
def get_label_assignment(
    ...
    if user_selected_list_item_label is not None:
        user_label = label_embeddings.getLabelByID(user_selected_list_item_label.label)
        ...
        for item_id in details.shoppingListItemIds:
            try:
                ...
                if not list_item.labelId:
                    list_item = assign_label_to_list_item(list_item, session, labelEmbeddings)
                    mealieShoppingList.updateListItem(list_item)
                get_repositories(session).listItemRepo.storeLabelAssignment(list_item)
